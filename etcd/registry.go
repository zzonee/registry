...
func NewRegistry(conf clientv3.Config) (registry.Registry, error) {
	r := &etcdReg{
		conf:    conf,
		options: &registry.Options{},
...
func (r *etcdReg) Register(ops ...registry.Option) {
	for _, o := range ops {
		o(r.options)
	}
...
func (r *etcdReg) register() error {
	...
	defer func() {
		if err != nil {
			log.Printf("register err:%v, step:%d, options:%v", err, step, r.options)
		} else {
			log.Printf("register uid:%s, options:%v", r.uid, r.options)
		}
	}()
